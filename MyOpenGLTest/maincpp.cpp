...
void mouse_pushbutton_callback(GLFWwindow* window, int button, int action, int mode);
void Do_Movement();
...
bool ifShowLine = false;

//Adding AckBall .....
GLfloat r = 2.0f;
glm::mat4 RMatrix(1.0f); // rotation matrix
void dragBall(GLfloat ax, GLfloat ay, GLfloat bx, GLfloat by);
...
int main()
{
	...
	glfwSetMouseButtonCallback(window, mouse_pushbutton_callback);
	...
	Shader shader("object.vert", "object.frag");
	...
	Model lightBulb("./resources/nanosuit.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (GLuint i = 0; i < 2; i++)
		{
			...
			lamp_model = glm::scale(lamp_model, glm::vec3(0.3f, 0.3f, 0.3f)); // Downscale lamp object (a bit too large)
			glUniformMatrix4fv(glGetUniformLocation(lampShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(lamp_model));
...
void mouse_move_callback(GLFWwindow* window, double xpos, double ypos)
{
	if (isDragged && !isDragFinished)
	{
		dragBall(lastX, lastY, xpos, ypos);
	}
	else if (!isDragged && isDragFinished)
	{
		lastX = xpos;
		lastY = ypos;
		isDragFinished = false;
...
void dragBall(GLfloat ax, GLfloat ay, GLfloat bx, GLfloat by)
{
	...
	RMatrix = glm::rotate(RMatrix, theta, glm::vec3(glm::inverse(model * view) * glm::vec4(N, 0.0f)));
}